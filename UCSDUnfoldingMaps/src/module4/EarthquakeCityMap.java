...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes(List<PointFeature> earthquakes) {
		java.util.Map<String, Integer> quakesInCountries = new java.util.HashMap<String, Integer>();
		for (PointFeature feature : earthquakes) {
			String country = null;
			try {
				country = (String) feature.getProperty("country");
			} catch (Exception e) {
				continue;
			}

			if (!(country == null)) {
				quakesInCountries.merge(country, 1, Integer::sum);
			} else {
				quakesInCountries.merge("Water", 1, Integer::sum);
			}
		}

		for (String s : quakesInCountries.keySet()) {
			if (!s.equals("Water")) {
				System.out.println("There were " + quakesInCountries.get(s) + " quakes in " + s);
			}
		}
		System.out.println("There were " + quakesInCountries.get("Water") + " quakes in the water");
	}
